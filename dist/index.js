(() => {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const splitTags = function() {
      ...
      const attr2 = (defaultVal, attrVal) => attrVal ?? defaultVal;
      const tags = document.querySelectorAll(`[${TAGS}]`);
      tags.forEach((item2) => {
        if (!item2) return;
        const className = attr2(CLASS_DEFAULT, item2.getAttribute(CLASS_ATTR));
        const pageTarget = attr2(PAGE_TARGET_DEFAULT, item2.getAttribute(PAGE_TARGET_ATTR));
        const filterCategory = attr2(FILTER_CATEGORY_DEFAULT, item2.getAttribute(FILTER_CATEGORY_ATTR));
        const fullTextString = item2.textContent || "";
        const itemTextArray = fullTextString.split(",");
        const tagWrap = document.createElement("div");
        tagWrap.className = TAG_WRAP_CLASS_DEFAULT;
        item2.parentNode.insertBefore(tagWrap, item2.nextSibling);
    ...
    function formatNumbers() {
      ...
      const items = document.querySelectorAll(ITEM);
      items.forEach((item2) => {
        let rawText = item2.textContent.trim().replace(/[^0-9.]/g, "");
        let number = parseFloat(rawText);
        if (isNaN(number)) return;
        const formatWithLetters = attr(true, item2.getAttribute(FORMAT_WITH_LETTERS));
        let formattedNumber;
        if (formatWithLetters) {
          if (number >= 1e6) {
            formattedNumber = (number / 1e6).toFixed(2).replace(/\.?0+$/, "") + "M";
          } else if (number >= 1e4) {
        ...
        item2.textContent = "$" + formattedNumber;
      });
    }
